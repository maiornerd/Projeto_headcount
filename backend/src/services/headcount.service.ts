...
    }
    const newEmployeesData = rows.map(row => {
      if (!row[matriculaIndex]) return null; 
      return {
        matricula: String(row[matriculaIndex]),
        nome: String(row[nomeIndex]),
        status: String(row[statusIndex]),
        funcao_codigo: String(row[funcaoCodigoIndex] || 'N/A'),
...
        prisma.employee.deleteMany({}), // APAGA TUDO
        prisma.employee.createMany({ // CRIA OS NOVOS
          data: newEmployeesData as any[] 
        })
      ]);
    } catch (dbError: any) {
...
  } // Fim do getDashboardData

} // <-- FIM DA CLASSE 'HeadcountService'